...
const sinonVersion = 'sinon-9.2.4.js';

const patchAllure = (folder) => {
  /**
   * Read all files,
   * create server.js,
   * then run server.js,
  ...
  for (const file of filesShouldBe) {
    if (!fs.existsSync(`${folder}/${file}`)) {
      throw new Error(`ERROR: file ${folder}/${file} doesnt exist, but it should!`);
    }
  }
  ...
  const serverFolder = `${folder}/server.js`;

  fs.writeFileSync(serverFolder, `
  ...
  fs.appendFileSync(serverFolder, 'var server_data={\n');
  for (const d of data) {
    let content;
    if (d.base64) {
    ...
    fs.appendFileSync(serverFolder, `"${d.url.replaceAll(sep, '/')}": "${content}", \n`);
  }
  fs.appendFileSync(serverFolder, '};\n');

  fs.appendFileSync(serverFolder, '    var server = sinon.fakeServer.create();\n');
  ...
  fs.appendFileSync(serverFolder, 'server.autoRespond = true;');
  ...
  console.log(`> Copying file ${sinonVersion} into folder...`);
  fs.copyFileSync(`${__dirname}${sep}${sinonVersion}`, `${folder}${sep}${sinonVersion}`);
  console.log(`${sinonVersion} is copied`);
  ...
  if (!/sinon-9.2.4.js/.test(indexHtml)) {
    console.log(`> Patching index.html file to make it use ${sinonVersion} and server.js`);
  ...
  for (const tag of soup.findAll('script')) {
    if (tag.attrs && tag.attrs.src && !tag.attrs.src.includes('googletagmanager')) {
  ...
  console.log(`Done. Complete file size is: ${completeFileSize} bytes. It took ${(performance.now() - startTime) / 1000}s.`);
};
